...
//The margins need to be defined 

//We are defining an array (var margin) because we can use margin.top/.bottom/etc. to easily call these values 
...
//The actual chart is going to occupy an area inside the svg which will be shifted to the center based on the  margins
...
d3.csv("/assets/data/data.csv") //use d3 to read csv file using D3 Version 5.5 

   .then(function (raw_data) { // convert the data from objects to js array 

      console.log(raw_data)

      //we need to iterate through the data using i as the index 
      ...
      for (var i = 0; i < raw_data.length; i++) { //defining the function: i is initially set to 0 and cannot exceed the amount of entries in the raw data, the value of i increases as the function loops 
         ...
         console.log(i, raw_data[i].state, raw_data[i].poverty, raw_data[i].healthcare); //prints these entries from the array to console

         console.log(i, raw_data[i].obesity, raw_data[i].income); //prints these entries from the array to console 

      }

      //call two separate arrays in which we store the values taken from the .csv file
      ...
      raw_data.forEach(function (data) { //Looping through all of the data -- forEach() calls a function once for each element in an array, in order
         ...
         data.obesity = +data.obesity; //Every time the forEach() is applied to an item in the "obesity" column, it adds the value to the array data.obesity

         data.income = +data.income; //same as above except we are adding each item from the "income" column to the income array

         console.log(data.income, data.obesity) //logs these values to the console
      ...
      y_scale.domain([25000, //This first argument [0, ] sets the minimum of y_scale, where the y_axis begins
      ...
      containerchart
         ...
         .attr('cx', function (data, index) {
            ...
            return x_scale(data.obesity); //the x_scale will be scaled relative to the values in obesity

         })

         .attr('cy', function (data, index) {

            return y_scale(data.income); //the y_scale will be scaled relative to the values in income
      ...
      svg.selectAll(".dot")
         ...
         .text(function (data) { return data.abbr; }) //state abbreviation will show up inside circles
